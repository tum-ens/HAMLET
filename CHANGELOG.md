...
- **The golden master can pin more than one scenario.** `tests/e2e/test_golden_master.py` held its
  ...
  have ever covered the grid stage. Scenarios are now a list, parametrised so each is run once per
  session, with its reference at `tests/e2e/golden/<scenario>.json`. Adding one is appending a
