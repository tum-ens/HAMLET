...
class Scenario:
    ...
    get_structures(path_specification: str = None, scenario_name: str = None) -> None
    ...
    def __gen_pv(self, list_plant_specs: list, participant_type: str) -> None:
        ...
        if participant_type == "prosumer":
            ...
            hh_consumption = next(item["annual_consumption"] for item in list_plant_specs if item["type"] == "inflexible_load")
    ...
    def __create_hh_files(self, **kwargs) -> None:
        ...
        filenames_hh = os.listdir(f'{self.path_input_data}/prosumers/inflexible_load/')
        ...
        filename_hh = f"{self.path_input_data}/prosumers/inflexible_load/{filenames_hh[idx]}"
    ...
    def __create_hp_files(self, **kwargs) -> None:
        ...
        hh_plant = next(plant for plant in account["list_plant_specs"] if plant["type"] == "inflexible_load")
        annual_consumption = hh_plant["annual_consumption"]

        # Read respective household time series from input data directory
        filename_hh = next(household for household in os.listdir(f'{self.path_input_data}/prosumers/inflexible_load/')
                           if str(annual_consumption) in household)
        filename_hh = f"{self.path_input_data}/prosumers/inflexible_load/{filename_hh}"
    ...
    def __reconfig_fcast_hh(self, prosumer: str) -> None:
        ...
        hh_plant = next(plant for plant in plant_dict if plant_dict[plant]["type"] == "inflexible_load")
        if self.config["prosumer"]["hh_fcast"] == "sarma":
            fcast_order = self.config["prosumer"]["hh_fcast_sarma_order"]
            num_param = sum(fcast_order) - fcast_order[6] - fcast_order[10]
            fcast_param = [1 / num_param] * num_param
    ...
    def __aggregate_prosumers(self, list_prosumers: list = None, without_plants_only: bool = False,
        ...
        for prosumer in list_prosumers:
            ...
            if not has_plant or not has_bat:
                ...
                hh_plant = next((plant for plant in plant_dict if plant_dict[plant]["type"] == "inflexible_load"), None)
                if hh_plant:
    ...
    def __gen_sum_aggregated_loads(self) -> None:
        ...
        for prosumer in os.listdir(f'{self.path_scenario}/prosumer/'):
            ...
            next((list_aggregated_plants.append([prosumer, plant]) for plant in plant_dict
                  if plant_dict[plant]["type"] == "inflexible_load" and plant_dict[plant]["fcast"] == "aggregator"), None)
    ...
    def __change_prosumer_setting(self, participant_type: str, setting: str, val, val_old, list_prosumers: list = None)\
        ...
        for prosumer in list_prosumers_all:
            with open(f"{self.path_scenario}/prosumer/{prosumer}/config_plants.json", "r") as read_file:
                plant_dict = json.load(read_file)
            plant = next((plant for plant in plant_dict if plant_dict[plant]["type"] == "inflexible_load"), None)
        ...
        if flag_error:
            try:
                ...
                raise Warning(f"The setting '{setting}' has too great of an influence on the entire simulation "
                              ...
                              f"Please create a new scenario using 'get_structures()' instead.")
    ...
    def __gen_dep_bool_list(list_bool: list, share_1s: float) -> list:
        ...
        n_1s = round(sum(list_bool) * share_1s)
