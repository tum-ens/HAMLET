...
class ScenarioAnalyzer:
    ...
    def __init__(self, path_results, save_figures: bool = False, show_figures: bool = True, start_time: int = None,
                 end_time: int = None):
        """initializer
        ...
        self.start_time = start_time if start_time is not None else self.__min_max_timestamp("min")
        self.end_time = end_time if end_time is not None else self.__min_max_timestamp("max")
    ...
    def __mcp_ex_ante(self, type_market, column_price) -> None:
        ...
        df_market_results = pd.read_csv(f"{self.path_results}/db_snapshot/results_market_{type_market}.csv",
                                        index_col=0)
        df_market_results = df_market_results[df_market_results[db_p.TS_DELIVERY] <= self.end_time]
    ...
    def plot_balance(self) -> None:
        ...
        df_transactions = pd.read_csv(f"{self.path_results}/db_snapshot/{db_p.NAME_TABLE_LOGS_TRANSACTIONS}.csv",
                                      index_col=0)
        df_transactions = df_transactions[df_transactions[db_p.TS_DELIVERY] <= self.end_time]
    ...
    def plot_weather(self) -> None:
        """plots the the most relevant weather parameters
        ...
        print("*** CREATING PLOT OF WEATHER ***")
        plt.close("all")

        # Read weather file and convert columns accordingly
        df = pd.read_feather(f"{self.path_results}/weather/weather.ft")
        df = df[df["ts_delivery_current"] == df["ts_delivery_fcast"]]
        print(self.start_time, self.end_time)
        df = df[(df["ts_delivery_current"] >= self.start_time) & (df["ts_delivery_current"] <= self.end_time)]
        df["ts_delivery_current"] = pd.to_datetime(df["ts_delivery_current"], unit="s", utc=False)
        tz = timezone(self.config["simulation"]["sim_start_tz"])
        df["ts_delivery_current"] += tz.utcoffset(df["ts_delivery_current"].iloc[0])
        df[["temp", "temp_feels_like", "temp_min", "temp_max"]] -= 273.15
        df.set_index("ts_delivery_current", inplace=True, drop=True)
        df[["temp", "wind_speed", "ghi"]].plot.line(subplots=True)
        plt.show()
    ...
    def __min_max_timestamp(self, val: str) -> int:
        ...
        if val == "min":
            return min(db_settlement_status[db_settlement_status[db_p.STATUS_SETTLEMENT_COMPLETE] == 1]
                       [db_p.TS_DELIVERY])
        elif val == "max":
            return max(db_settlement_status[db_settlement_status[db_p.STATUS_SETTLEMENT_COMPLETE] == 1]
                       [db_p.TS_DELIVERY])
        else:
            raise Warning("'val' only takes 'min' or 'max' as argument.")
